...
class WaitFiveSecondsSensor(BaseSensorOperator):
    ...
    def execute(self, context: Context) -> None:
        deferrable_supported = not version.version.startswith("2.5") and not version.version.startswith("2.6")
...
with DAG('triggerer', schedule_interval=None, start_date=datetime(2022, 1, 1), catchup=False) as dag:
    ...
    wait_five = WaitFiveSecondsSensor(
        task_id='triggerer'
    )
