...
- [x] 1. Implement CredentialExtractor utility
  - [x] 1.1 Create `assets/dags/mwaa_dr/framework/credential_extractor.py` with the `CredentialExtractor` class
  ...
  - [x] 1.2 Write unit tests for CredentialExtractor
  ...
  - [x] 1.3 Write property test for DB_SECRETS credential extraction
    - **Property 1: DB_SECRETS credential extraction preserves components**
    - **Validates: Requirements 1.1**

  - [x] 1.4 Write property test for SQLAlchemy connection string credential extraction
    - **Property 2: SQLAlchemy connection string credential extraction preserves components**
    - **Validates: Requirements 1.2**

- [x] 2. Implement MwaaRestApiClient utility
  - [x] 2.1 Create `assets/dags/mwaa_dr/framework/mwaa_rest_api_client.py` with the `MwaaRestApiClient` class
  ...
  - [x] 2.2 Write unit tests for MwaaRestApiClient (mocked HTTP)
...
- [x] 4. Implement GlueDRFactory base class
  - [x] 4.1 Create `assets/dags/mwaa_dr/framework/factory/glue_dr_factory.py` with the `GlueDRFactory` class extending `BaseDRFactory`
  ...
  - [x] 4.2 Implement `create_backup_dag()` in GlueDRFactory
  ...
  - [x] 4.3 Implement `backup_variables_via_api()` and `backup_connections_via_api()` in GlueDRFactory
  ...
  - [x] 4.4 Implement `create_restore_dag()` in GlueDRFactory
  ...
  - [x] 4.5 Implement `restore_variables_via_api()` and `restore_connections_via_api()` in GlueDRFactory
  ...
  - [x] 4.6 Implement `create_cleanup_dag()` in GlueDRFactory
  ...
  - [x] 4.7 Write unit tests for GlueDRFactory (mocked AWS)
  ...
  - [x] 4.8 Write property test for Glue connection naming
    - **Property 13: Glue connection naming follows environment pattern**
    - **Validates: Requirements 2.4**

  - [x] 4.9 Write property test for S3 path construction
    - **Property 3: S3 path construction follows naming conventions**
    - **Validates: Requirements 3.2, 4.2, 7.3**

  - [x] 4.10 Write property test for restore strategy behavior
    - **Property 11: Restore strategy determines correct API behavior**
    - **Validates: Requirements 11.5, 11.6, 11.7**

- [x] 5. Implement DRFactory_3_0 version-specific factory
  - [x] 5.1 Create `assets/dags/mwaa_dr/v_3_0/__init__.py` and `assets/dags/mwaa_dr/v_3_0/dr_factory.py` with `DRFactory_3_0` extending `GlueDRFactory`
  ...
  - [x] 5.2 Write unit tests for DRFactory_3_0
...
- [x] 7. Implement Glue scripts
  - [x] 7.1 Create `assets/glue_scripts/mwaa_metadb_export.py`
  ...
  - [x] 7.2 Create `assets/glue_scripts/mwaa_metadb_import.py`
  ...
  - [x] 7.3 Create `assets/glue_scripts/mwaa_metadb_cleanup.py`
  ...
  - [x] 7.4 Write property test for job summary correctness
    - **Property 6: Job summary contains all processed tables**
    - **Validates: Requirements 3.5, 12.4**

  - [x] 7.5 Write property test for cleanup preserving protected records
    - **Property 8: Cleanup preserves protected records**
    - **Validates: Requirements 5.4**

- [x] 8. Update config.py for Airflow 3.x support
  - [x] 8.1 Update `config.py`
  ...
  - [x] 8.2 Write unit tests for config changes
...
- [x] 9. Update CDK stacks for Glue IAM and script deployment
  - [x] 9.1 Update `lib/stacks/mwaa_primary_stack.py`
  ...
  - [x] 9.2 Update `lib/stacks/mwaa_secondary_stack.py`
  ...
  - [x] 9.3 Write CDK assertion tests for Glue resources
...
- [x] 10. Update entry point DAGs for version 3.x routing
  - [x] 10.1 Update `assets/dags/mwaa_dr/backup_metadata.py`
  ...
  - [x] 10.2 Update `assets/dags/mwaa_dr/restore_metadata.py`
    - Add `elif airflow_version.startswith("3."):` block before the `else` fallback
    - Import and instantiate `DRFactory_3_0` from `mwaa_dr.v_3_0.dr_factory`
    - Preserve all existing 2.x version branches unchanged
    - _Requirements: 6.4, 10.1_

  - [x] 10.3 Update `assets/dags/mwaa_dr/cleanup_metadata.py`
    - Add `elif airflow_version.startswith("3."):` block before the `else` fallback
    - Import and instantiate `DRFactory_3_0` from `mwaa_dr.v_3_0.dr_factory`
    - Preserve all existing 2.x version branches unchanged
    - _Requirements: 6.4, 10.1_

  - [x] 10.4 Write property test for version routing
...
- [x] 11. Backward compatibility verification
  - [x] 11.1 Run existing unit tests to verify no regressions
